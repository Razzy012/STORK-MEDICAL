<script>
    ...
    const change = e => {
        ...
        if (inputs[0].value.trim() === inputs[1].value.trim()) {
            ...
            updatePassword(getAuth().currentUser, password).then(() => reset(e))
                .catch ((e) => {
...
<div data-newUser class="border p-6 rounded bg-white border-frame-border">
    ...
    <div class="mt-7 m-auto max-w-fit">
        <div class="mr-1 inline-block"><Button onclick={change}>Změnit</Button></div><Button onclick={reset} width='85.25px'>Zrušit</Button>
    </div>
</div>
