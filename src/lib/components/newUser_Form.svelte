<script lang="ts">
	...
	export let props: {
		usersDocs?: { roles: any; name: any; uid: string }[];
	...
	const defaultProps = {
		userDocs: [],
		submitAction: () => removePopup(),
		cancelAction: () => removePopup()
	};
		...
		if (inputs[1].value.trim() === inputs[2].value.trim()) {
			...
			createUserWithEmailAndPassword(auth, email, password)
				.then(async (userCredential) => {
					...
					e_email = false;
					e_password = false;
					props.submitAction(roles, email, user.uid);
				})
				.catch((e) => {
					if (
...
<div data-newUser class="border p-6 rounded bg-white border-frame-border">
	...
	<input
		...
		class:e_password
	/><br />
	<div class="m-auto mt-4 max-w-fit">
        <Select {options} bind:choosed buttonCSS={'h-10'}/>
    </div>
